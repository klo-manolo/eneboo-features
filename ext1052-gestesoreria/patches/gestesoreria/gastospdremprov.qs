/***************************************************************************
                 gastospdremprov.qs  -  description
                             -------------------
    begin                : vier nov 04 2011
    ...
    switch (fN) {
        case "importeeuros":
            ...
            if (cursor.valueBuffer("coddivisa") == this.iface.divisaEmp && tasaConv == 1){
                ...
                valor = parseFloat(util.roundFieldValue(valor, "gastospdremprov", "importeeuros"));
            }
            if (!valor) valor = 0;
            break;
        ...
        case "idsubcuentadefecto":
            var idS = util.sqlSelect("co_subcuentas s INNER JOIN cuentasbanco c on c.codsubctacomision = s.codsubcuenta INNER JOIN remesasprov r ON r.codcuenta = c.codcuenta","s.idsubcuenta","s.codejercicio='"+this.iface.ejercicioActual+"' AND r.idremesa = '"+this.iface.curRelacionado.valueBuffer("idremesa")+"'","co_subcuentas,cuentasbanco,remesasprov");
            if (idS){
                valor = idS;
            }else {
                valor = 0;
