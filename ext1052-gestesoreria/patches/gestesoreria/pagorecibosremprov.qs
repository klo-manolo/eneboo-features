/**************************************************************************
                 pagorecibosremprov.qs  -  description
                             -------------------
...
function interna_validateForm():Boolean
{
    ...
    if (cursor.valueBuffer("tipo") == "Anticipo" && (cursor.isNull("documento") || cursor.valueBuffer("documento") == "")) {
        MessageBox.warning(util.translate("scripts", "Para generar un registro de tipo Anticipo, es obligatorio indicar el documento"), MessageBox.Ok, MessageBox.NoButton);
        return false;
    }
...
function oficial_filtroRecibos():String{
    ...
    if (cursor.valueBuffer("codproveedor")) {
        where += " AND r.codproveedor = '"+cursor.valueBuffer("codproveedor")+"'";
    }
    
    if (cursor.valueBuffer("tipo") == "Pago"){
        where += " AND (r.estado ='Remesado' OR r.estado ='Confirmado')";
    }else if (cursor.valueBuffer("tipo") == "Anticipo"){
        where += " AND r.estado ='Remesado'"
    }

    var qryRecibos:FLSqlQuery = new FLSqlQuery();
    qryRecibos.setTablesList("recibosprov,remesasprov,recibosprovremesa");
    qryRecibos.setSelect("r.idrecibo");
    qryRecibos.setFrom("recibosprov r INNER JOIN recibosprovremesa rm ON r.idrecibo = rm.idrecibo INNER JOIN remesasprov m ON m.idremesa = rm.idremesa AND m.tipoconta='202' AND m.idremesa = r.idremesa");
...
function oficial_bufferChanged(fN:String)
{
    ...
    switch (fN) {
        ...
        case "tipo":
            ...
            this.iface.tbnNinguno_clicked();
            this.iface.tbnNingunoSel_clicked();
            cursor.setNull("recibos");
            this.iface.refrescarTablas();
            break;
    }
}
