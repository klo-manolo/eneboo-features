...
{
	var util:FLUtil = new FLUtil();
	var cadena:String = util.sqlSelect("remesasprov", "idremesa", "1 = 1 ORDER BY idremesa DESC");
	var valor:Number;
	if (!cadena)
		valor = 1;
	else
	...
	function oficial_excluirReciboRemesa(idRecibo:String, idRemesa:String):Boolean
{
	...
	/*
	var cuentaValida:String = util.sqlSelect("recibosprov r LEFT OUTER JOIN cuentasbcocli c ON r.codcliente = c.codcliente", "r.idrecibo", "idrecibo = " + idRecibo + " AND (r.codcuenta = c.codcuenta OR r.codcuenta = '' OR r.codcuenta IS NULL)", "reciboscli,cuentasbcocli");
	if (!cuentaValida) {
	...
	*/
	...
	var idUltimaRemesa:String = util.sqlSelect("pagosdevolprov", "idremesa", "idrecibo = " + idRecibo + " ORDER BY fecha DESC, idpagodevol DESC")
